    ...
    document.querySelectorAll('.quiz-option').forEach(button => {
        button.addEventListener('click', event => {
            ...
            if (currentPage.startsWith('quiz') && currentPage.endsWith('.html')) {
                // クイズページの場合は次のアンサーページへ
                const currentNumber = parseInt(currentPage.replace('quiz', '').replace('.html', ''), 10);
                nextPage = `answer${currentNumber}.html`;
            } else if (currentPage.startsWith('answer') && currentPage.endsWith('.html')) {
                // アンサーページの場合は次のクイズページへ
                const currentNumber = parseInt(currentPage.replace('answer', '').replace('.html', ''), 10);
                if (currentNumber >= 1 && currentNumber < 10) {
                    nextPage = `quiz${currentNumber + 1}.html`;
                } else if (currentNumber === 10) {
                    nextPage = 'index.html'; // 最後はトップページに戻る
                }
            } else {
                // デフォルトとしてquiz2.htmlに遷移
                nextPage = 'quiz2.html';
            }
